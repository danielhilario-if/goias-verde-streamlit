...
_Q10_PRESET_KEY = "Q10_TS_FCO2"
_Q10_TREF = 10.0  # graus Celsius


def _localized_preset_label(internal_key: str) -> str:
    mapping = {
        "Temperatura do Solo x Fluxo de CO2": "regression.preset.ts_co2",
        "Umidade do Solo x Fluxo de CO2": "regression.preset.swc_co2",
        "Temperatura do Solo x Fluxo de CH4": "regression.preset.ts_ch4",
    }
    return t(mapping.get(internal_key, "regression.preset.ts_co2"))


def _q10_fit(temperature: pd.Series, flux: pd.Series, tref: float = _Q10_TREF) -> tuple[float, float, float]:
    """Ajuste log-linear: log(flux) = log(R0) + ((T - Tref)/10) * log(Q10).

    Retorna (R0, Q10, R2) ou levanta ValueError se nao houver dados suficientes.
    """
    mask = (flux > 0) & temperature.notna()
    x = temperature[mask].to_numpy()
    y = flux[mask].to_numpy()
    if len(x) < 5:
    ...
    q10 = float(np.exp(log_q10))

    pred = log_r0 + z * log_q10
...
def render():
    ...
    # Q10 preset (only if TS_2 and FCO2 present)
    if "TS_2 initial_value" in df.columns and "FCO2_DRY" in df.columns:
        available_presets.append((_Q10_PRESET_KEY, t("regression.preset.q10")))
    ...
    if selected_label != none_label:
        ...
        if internal == _Q10_PRESET_KEY:
            try:
                r0, q10, r2 = _q10_fit(df["TS_2 initial_value"], df["FCO2_DRY"])
            except ValueError:
                st.warning(t("regression.q10.warn_low"))
            else:
                work = df[["TS_2 initial_value", "FCO2_DRY"]].dropna()
                work = work[work["FCO2_DRY"] > 0]
                fig_q10, ax_q10 = plt.subplots(figsize=(8, 5))
                ax_q10.scatter(work["TS_2 initial_value"], work["FCO2_DRY"], alpha=0.5, s=22, color="#0f766e")
                xs = np.linspace(work["TS_2 initial_value"].min(), work["TS_2 initial_value"].max(), 200)
                ys = r0 * np.power(q10, (xs - _Q10_TREF) / 10.0)
                ax_q10.plot(xs, ys, color="#b91c1c", linewidth=2)
                ax_q10.set_xlabel("TS_2 initial_value")
                ax_q10.set_ylabel("FCO2_DRY")
                ax_q10.set_title(t("regression.preset.q10"))
                st.pyplot(fig_q10)
                plt.close(fig_q10)
                st.caption(
                    t(
                        "regression.q10.caption",
                        tref=int(_Q10_TREF),
                        r0=r0,
                        q10=q10,
                        r2=r2,
    ...
    default_hue = "Época" if "Época" in cat_cols else none_label
